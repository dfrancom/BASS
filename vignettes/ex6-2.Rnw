...
mod <- bass(x, y, xx.func = x.func, nmcmc = 110, nburn = 100, 
            thin = 1, h2 = 1e250, save.yhat = F, temp.ladder = rep(1, 10), 
...
<<ex6plot1, fig.cap='BASS prediction performance - pollutant spill model.',dev='png'>>=
...
<<ex6plot2, fig.height=10, fig.width=12, out.width='\\linewidth', fig.cap='BASS prediction in space and time - pollutant spill model.'>>=
pp <- pred[, 1, ]
ylim <- range(y)
par(mfrow=c(2, 3))
for(i in 1:length(s)) {
...
<<ex6plot3, cache=T, fig.height=6, fig.width=12, out.width='.8\\linewidth', fig.cap='Sensitivity indices as a function of time - pollutant spill model.'>>=
