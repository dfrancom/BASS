...
The BASS framework builds on multivariate adaptive regression splines (MARS) from \cite{friedman1991multivariate}.  A well-developed software implementation of the MARS model is available in the \proglang{R} package \pkg{earth} \citep{earth}.  The Bayesian version of MARS (BMARS) was first developed in \cite{denison1998bayesian}.  A \proglang{MATLAB} implementation of BMARS is available from the software website accompanying \cite{denison2002bayesian}.  
