...
birth_des_func<-function(curr,prior,data){
  ...
  cand.func<-genCandBasis(minInt=prior$minInt,maxInt=prior$maxInt.func,I.vec=curr$I.vec.func,z.vec=curr$z.vec.func,p=data$pfunc,xxt=data$xxt.func,q=prior$q,xx.unique.ind=data$unique.ind.func,vars.len=data$vars.len.func)
  ...
  alpha<- data$temp.ladder[curr$temp.ind]*(.5/curr$s2*(qf.cand.list$qf-curr$qf) + log(curr$lam) - log(curr$nc) + log(data$death.prob.next/data$birth.prob) - cand.des$lbmcmp - cand.func$lbmcmp)
...
death_des_func<-function(curr,prior,data){
  ...
  I.star.des[curr$n.int.des[basis]]<-I.star.des[curr$n.int.des[basis]+1]-1
  ...
  I.star.func[curr$n.int.func[basis]]<-I.star.func[curr$n.int.func[basis]+1]-1
  ...
  lpbmcmp<-0
  if(curr$n.int.des[basis]>0){
    lpbmcmp<-lpbmcmp+logProbChangeMod(curr$n.int.des[basis],curr$vars.des[basis,1:curr$n.int.des[basis]],I.vec.des,z.vec.des,data$pdes,data$vars.len.des,prior$maxInt.des)
  }

  if(curr$n.int.func[basis]>0){
    lpbmcmp<-lpbmcmp+logProbChangeMod(curr$n.int.func[basis],curr$vars.func[basis,1:curr$n.int.func[basis]],I.vec.func,z.vec.func,data$pfunc,data$vars.len.func,prior$maxInt.func)
  }
...
change_des_func<-function(curr,prior,data){
  ...
  if(sum(cand.func$basis!=0)<prior$npart.des){
    return(curr)
  }
